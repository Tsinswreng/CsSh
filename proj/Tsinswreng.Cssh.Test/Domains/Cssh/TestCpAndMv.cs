...
/// Tests copying and moving complete file-system structures.
public partial class TestCssh{
	public void RegisterCpAndMv(ITestNode Node) {
	...
	/// Recursive copy must retain an empty directory as well as ordinary file content.
	public Task<object?> CpPreservesFilesAndEmptyDirectories(object? O) {
	...
	/// Moving a file should build a missing destination parent rather than requiring a preliminary Mkdir.
	public Task<object?> MvCreatesDestinationParent(object? O) {
		var Root = TestSupport.NewRoot();
		try {
	...
	public async Task<object?> AsyncCpAndMvNeedNoNullOptions(object? O) {
		var Root = TestSupport.NewRoot();
		try {
